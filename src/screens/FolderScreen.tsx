...
const FolderScreen: React.FC<FolderScreenProps> = ({navigation, route}) => {
  ...
  useEffect(() => {
    ...
    async function fetchMedia(folder_id: string, accountId: string) {
      ...
      if (response.success) {
        ...
        Alert.alert(
          'Error',
          response.message || 'An error occurred while fetching meida.',
        );
      }
      ...
      if (responseFolder.success) {
        ...
        Alert.alert(
          'Error',
          responseFolder.message || 'An error occurred while fetching meida.',
        );
      }
    }
  ...
  const handleSubmitForm = async () => {
    ...
    if (response.success) {
      ...
      Alert.alert(
        'Error',
        response.message || 'An error occurred while sharing folder.',
      );
      Toast.show({
        type: 'error',
        text1: 'oops!',
    ...
    <>
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.privacy}>
          {folderDetails.privacy ? <Text>Private</Text> : <Text>Public</Text>}
        </Text>
      </View>
      {canEdit ? (
      ...
      <BottomSheet
        ...
        <View style={styles.contentContainer}>
          <Text style={{color: COLORS.secondary, fontSize: SIZES.h4}}>
            Enter the phone number with you want to share 🎉
          </Text>
