...
const FoldersList: React.FC<FoldersListProps> = ({
    ...
    const fetchFolder = async () => {
      ...
      if (response2.success) {
        ...
        Alert.alert(
          'Error',
          response2.message ||
            'An error occurred while fetching shared folders.',
        );
      }
    };
    fetchFolder();
  ...
  const loadThumbnail = async (folder_id: string) => {
    ...
    if (response3.success) {
      ...
      Alert.alert(
        'Error',
        response3.message || 'An error occurred while fetching meida.',
      );
    }
  };
      ...
      <TouchableOpacity
        ...
        <ImageBackground
          ...
          source={{
            uri:
              BASE_URL +
              `/uploads/${folder.account_id}/${folder.folder_id}/${folder.image_id}.png`,
            headers: {Authorization: `bearer ${jwtToken}`},
          }}
          blurRadius={5}>
          <Text style={[styles.folderListTabText]}>{folder.folder_name}</Text>
        </ImageBackground>
      </TouchableOpacity>
    );
  };
    ...
    <View style={{}}>
      ...
      <View style={styles.folderList}>
        {folders ? (
          ...
          <Text style={[styles.darkText]}>no folders</Text>
        )}
      </View>
